...
// Plain notification (no calendar attachment): used when a slot is free but
// the booking cannot be completed automatically and must be made by hand.
export const sendAlert = async ({ from, to, subject, text }) => {
  const recipients = Array.isArray(to) ? to : [to]

  const transporter = buildTransport()
  if (!transporter) {
    console.log('No email transport configured, skipping alert email')
    return
  }
  if (!from || recipients.length === 0) {
    console.log('Email sender or recipients missing, skipping alert email')
    return
  }
  ...
  try {
    ...
    console.log('Error while sending alert email:', err)
  }
}
